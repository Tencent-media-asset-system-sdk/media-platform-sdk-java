 ...
 */
public class TIHttpClient extends AbstractRpcClient {

    private static final Logger logger = LoggerFactory.getLogger(TIHttpClient.class);

    private CloseableHttpClient httpClient;

    public TIHttpClient(ProtocolConfig config) {
        setConfig(config);
    }
    ...
    public <T> ConsumerInvoker<T> createInvoker(ConsumerConfig<T> consumerConfig) {
        return new TIConsumerInvoker<>(consumerConfig, protocolConfig);
    }
